...
- **Every proposed task is a record, not a sentence.** Each task is structured: a title, a `why_now` naming the mission link, a `verify` command that actually runs, whether it's self-doable, which files it writes, its origin (directed or discovery), and — for discovery — the cited signal. That structure is what lets the gate check it mechanically instead of trusting prose.
